...
export default function Website() {
  ...
  React.useEffect(() => {
    ...
    setMeta(
      "keywords",
      "Spielhalle Vegas, PG Pro Games GmbH, Spielhalle Baumholder, Spielhalle Montabaur, Spielothek Baumholder, Spielothek Montabaur, Automatenaufstellung Gastronomie, Automatenaufsteller Hessen, Automatenaufsteller Rheinland-Pfalz, Geldspielgeräte Gastronomie"
    );
        ...
        {loading && (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute h-72 w-72 rounded-full border border-amber-300/10"
            />
            <motion.div
            ...
            >
              ...
              <motion.div
                ...
                className="mx-auto mt-8 h-[2px] bg-gradient-to-r from-transparent via-amber-300 to-transparent"
              />
              <div className="mt-6 text-sm font-black uppercase tracking-[0.45em] text-amber-300">
                Spielhalle Vegas
              </div>
            </motion.div>
          </motion.div>
      ...
      <main className="min-h-screen overflow-x-hidden bg-[#050403] font-sans text-white selection:bg-amber-300 selection:text-black">
        <header className="sticky top-0 z-50 border-b border-amber-400/10 bg-[#050403]/90 backdrop-blur-2xl">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
            ...
            <a href="#kontakt" className="hidden rounded-xl bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600 px-5 py-3 text-xs font-black uppercase tracking-wide text-black shadow-lg shadow-amber-500/20 transition hover:scale-[1.03] md:inline-flex">
              Kontakt
            </a>
          ...
          {mobileMenu && (
            <div className="border-t border-amber-400/10 bg-[#050403]/95 px-6 py-5 lg:hidden">
              <div className="flex flex-col gap-4 text-sm font-bold uppercase tracking-wide text-zinc-300">
                {navItems.map((item) => (
                  <a key={item.href} href={item.href} onClick={() => setMobileMenu(false)} className="transition hover:text-amber-300">
                    {item.label}
                  </a>
                ))}
              </div>
          ...
          <div className="relative mx-auto max-w-7xl px-6 py-24 md:py-32">
            <motion.div {...fadeUp} className="mx-auto max-w-5xl text-center">
              ...
              <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.15 }} className="mt-10 flex flex-col justify-center gap-4 sm:flex-row">
                <a href="#standorte" className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600 px-8 py-4 text-sm font-black uppercase tracking-wide text-black shadow-xl shadow-amber-500/20 transition hover:scale-[1.02]">
                  Unsere Standorte <MapPin size={18} />
                </a>
                <a href="#gastronomie" className="inline-flex items-center justify-center gap-2 rounded-xl border border-amber-300/25 bg-black/35 px-8 py-4 text-sm font-black uppercase tracking-wide text-white backdrop-blur transition hover:scale-[1.02] hover:border-amber-300/50 hover:bg-amber-300/10">
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2">
              {locations.map((location, index) => (
                <motion.div key={location.city} {...fadeUp} transition={{ duration: 0.6, delay: index * 0.1 }} className={`${premiumCard} p-8 transition hover:-translate-y-1 hover:border-amber-300/35`}>
                  ...
                  <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location.address)}`} target="_blank" rel="noreferrer" className="mt-5 inline-flex rounded-xl border border-amber-300/25 px-5 py-3 text-sm font-bold text-amber-100 transition hover:bg-amber-300 hover:text-black">
                    Route öffnen
                  </a>
                  ...
                  <div className="mt-6 overflow-hidden rounded-2xl border border-amber-300/10 bg-black/40">
                    <iframe title={`Google Maps ${location.city}`} src={`https://www.google.com/maps?q=${encodeURIComponent(location.address)}&output=embed`} className="h-[300px] w-full grayscale contrast-125 transition duration-700 hover:grayscale-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="rounded-2xl border border-amber-300/10 bg-black/35 p-6">
                ...
                <div className="space-y-4 text-zinc-300">
                  <div><div className="font-bold text-white">BZgA</div><div>www.check-dein-spiel.de</div></div>
                  <div><div className="font-bold text-white">Telefonberatung</div><div>0800 1 37 27 00</div></div>
                  <div><div className="font-bold text-white">Anonyme Hilfe</div><div>kostenfrei und vertraulich</div></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {protection.map((item, index) => (
                <motion.div key={item} {...fadeUp} transition={{ duration: 0.55, delay: index * 0.06 }} className="rounded-2xl border border-amber-300/10 bg-black/30 p-6 transition hover:bg-amber-300/[0.05]">
        ...
        <section className="bg-[#070605] px-6 py-24">
          <div className="mx-auto max-w-7xl">
            <motion.div {...fadeUp} className="max-w-3xl">
              <p className="font-black uppercase tracking-[0.22em] text-amber-300">Lokale Bereiche</p>
              <h2 className="mt-4 text-4xl font-black tracking-tight md:text-5xl">Regionen & Leistungen</h2>
              <p className="mt-5 text-lg leading-8 text-zinc-300">PG Pro Games GmbH ist Ihr Ansprechpartner für moderne Spielhallen und professionelle Automatenaufstellung in Rheinland-Pfalz und Hessen.</p>
            </motion.div>

            <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {localSeo.map((item) => (
                <motion.article whileHover={{ y: -6 }} key={item.title} className="rounded-[2rem] border border-amber-300/10 bg-white/[0.03] p-6 transition hover:border-amber-300/30 hover:bg-amber-300/[0.04]">
                  <div className="mb-4 h-1 w-16 rounded-full bg-gradient-to-r from-amber-300 to-yellow-600" />
          ...
          <div className={`relative mx-auto max-w-6xl ${premiumCard} p-8 backdrop-blur md:p-12`}>
            <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
              <motion.div {...fadeUp}>
                ...
                <p className="mt-5 max-w-2xl text-lg leading-8 text-zinc-300">Sie haben Fragen zu unseren Spielhallen, Standorten, Öffnungszeiten oder zur Automatenaufstellung? Kontaktieren Sie uns gerne.</p>
              ...
              <motion.div {...fadeUp} className="rounded-[2rem] border border-amber-300/10 bg-black/30 p-8">
                ...
                <p className="mt-3 leading-8 text-zinc-400">Für allgemeine Fragen, Kooperationen oder Automatenaufstellungen kontaktieren Sie uns direkt per E-Mail oder telefonisch.</p>
                <div className="mt-8 space-y-4">
                  <a href="mailto:info@pgprogames.de" className="flex items-center justify-between rounded-2xl border border-amber-300/10 bg-white/[0.04] px-6 py-5 transition hover:bg-amber-300/[0.08]">
                    <div><div className="text-sm text-zinc-500">E-Mail</div><div className="mt-1 text-lg font-bold text-white">info@pgprogames.de</div></div>
                    <ArrowRight size={20} className="text-amber-300" />
                  </a>
                  <a href="tel:+4917622398057" className="flex items-center justify-between rounded-2xl border border-amber-300/10 bg-white/[0.04] px-6 py-5 transition hover:bg-amber-300/[0.08]">
                    <div><div className="text-sm text-zinc-500">Telefon</div><div className="mt-1 text-lg font-bold text-white">+49 176 22398057</div></div>
        ...
        <footer className="border-t border-amber-300/10 bg-black px-6 py-10 text-sm text-zinc-500">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 md:flex-row">
            <div className="flex items-center gap-4">
              ...
              <div><div className="font-black tracking-[0.18em] text-white">PG PRO GAMES</div><div className="text-amber-300/70">Spielhalle Vegas</div></div>
            </div>
